...
@section('content')
<div data-page="pengajar-add-tp"
     data-csrf-token="{{ csrf_token() }}"
     data-mata-pelajaran-id="{{ $mataPelajaran->id }}"
     data-list-url="{{ url('/pengajar/tujuan-pembelajaran/' . $mataPelajaran->id . '/list') }}"
     data-store-url="{{ route('pengajar.tujuan_pembelajaran.store') }}"
     data-destroy-base-url="{{ url('/pengajar/tujuan-pembelajaran') }}">
    <div class="p-4 bg-white mt-14 shadow-lg rounded-lg">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            ...
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('pengajar.subject.copy_lm_tp', $mataPelajaran->id) }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                    Salin LM/TP
                </a>
