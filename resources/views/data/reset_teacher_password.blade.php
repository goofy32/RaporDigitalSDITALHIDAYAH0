            ...
            <div class="max-w-2xl">
                ...
                <div class="overflow-hidden bg-white border border-gray-200 rounded-lg shadow-sm">
                    <div class="px-5 py-4 border-b border-gray-100 bg-white">
                        <h1 class="text-2xl font-bold text-gray-900">Reset Password Guru</h1>
                        <p class="mt-1 text-sm text-gray-600">
                            Buat password sementara agar guru bisa login kembali.
                        </p>
                    </div>

                    <div class="p-5">
                        <div class="grid gap-3 px-4 py-3 mb-4 text-xs border border-gray-200 rounded-lg bg-gray-50 sm:grid-cols-3">
                            <div>
                        ...
                        <div class="px-4 py-3 mb-4 text-xs leading-5 text-gray-600 border border-gray-200 rounded-lg bg-gray-50">
                            <p>Password lama tidak dapat ditampilkan demi keamanan.</p>
                            <p>Setelah reset, guru wajib mengganti password saat login berikutnya.</p>
                        </div>
                        ...
                        <form method="POST" action="{{ route('teacher.reset-password.update', $teacher->id) }}" class="space-y-4">
                            @csrf
                            @method('PUT')

                            <div>
                                <label for="password" class="block text-sm font-medium text-gray-700">Password sementara untuk login awal</label>
                                <div class="flex flex-col gap-2 mt-1 sm:flex-row sm:items-start">
                                    <div class="relative w-full sm:max-w-sm">
                                        <input id="password" name="password" type="password" autocomplete="new-password" required
                                            class="reset-password-input block w-full pr-12 rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">
                                        <button type="button"
                                            class="absolute inset-y-0 right-0 z-10 flex items-center justify-center w-11 text-gray-500 hover:text-green-700 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-green-500"
                                    ...
                                    <button type="button" id="generate-password"
                                        class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-green-700 bg-green-50 border border-green-200 rounded-md sm:shrink-0 hover:bg-green-100">
                                        Buat Password
                                    </button>
                                </div>
